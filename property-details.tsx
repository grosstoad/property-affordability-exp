...
import BorrowingPowerResultsDisplay from "@/components/BorrowingPowerResultsDisplay"

// Import the new rate hook
...
export default function PropertyDetails({
  ...
  // Initialize the rate hook
  const rateHook = useRates();
  ...
  const [showBorrowingPowerResults, setShowBorrowingPowerResults] = useState(false)
  ...
  const [viewAffordability, setViewAffordability] = useState(false);
  ...
  const updateLoanAmount = (newPropertyValue: string, newDeposit: string) => {
    const propertyValueNum = parseCurrency(newPropertyValue);
    const depositNum = parseCurrency(newDeposit);
    
    // Calculate upfront costs first
    calculateUpfrontCosts(propertyValueNum, 0); // Pass 0 as initial loan amount
    
    // Calculate deposit for loan (deposit minus upfront costs)
    const depositForLoan = depositNum - upfrontCosts;
    
    // Calculate loan amount correctly
    const newLoanAmount = propertyValueNum - depositForLoan;
    const newLvr = (newLoanAmount / propertyValueNum) * 100;
    
    setLoanAmount(newLoanAmount);
    
    // Use the rate hook to find the best rate
    const rateInfo = rateHook.findBestRateFromOldParams(
      newLvr, // The hook accepts number or string
      interestRateType,
      repaymentType,
      loanPurpose,
      featureType,
    ...
    // Check if rateInfo exists before accessing properties
    if (rateInfo) {
      setInterestRate(rateInfo.rate.toString());
      setComparisonRate(rateInfo.comparisonRate.toString());
      
      // Calculate monthly payment based on the new values
      const payment = calculatePmt(newLoanAmount, parseFloat(rateInfo.rate.toString()), Number(loanTerm) * 12);
      setMonthlyPayment(`$${Math.round(payment).toLocaleString()}`);
    } else {
      // Fallback to default rates if no rate information is available
      setInterestRate("5.74");
      setComparisonRate("5.65");
      
      // Calculate monthly payment with default rate
      const payment = calculatePmt(newLoanAmount, 5.74, Number(loanTerm) * 12);
      setMonthlyPayment(`$${Math.round(payment).toLocaleString()}`);
    }
  };

  // Handle borrowing power calculation
  const handleBorrowingPowerCalculate = (values: any) => {
    console.log("Borrowing Power Form Data:", values)
    
    // Calculate current LVR
    const propertyValueNum = parseCurrency(propertyValue);
    const depositNum = parseCurrency(deposit);
    const depositForLoan = depositNum - upfrontCosts;
    const loanAmountRequired = propertyValueNum - depositForLoan;
    const currentLvr = (loanAmountRequired / propertyValueNum) * 100;
    
    // Get the appropriate interest rate based on LVR
    const lvrBasedRate = getMortgageChoiceFreedomRate(currentLvr);
    
    // Update the interest rate in the form values
    values.interestRate = lvrBasedRate;
    
    // Process form data with the updated interest rate
    const calculationInput = processFormData(values)
    ...
    // Set borrowing power modal visibility
    setShowBorrowingPowerModal(false)
    setIsBorrowingPowerModalOpen(false)
    
    // Show the borrowing power results section
    setShowBorrowingPowerResults(true)
    
    // Reset view affordability flag
    setViewAffordability(false)
  };
  ...
  // Update the handleLoanDetailsUpdate function to use the new rate system
  const handleLoanDetailsUpdate = (values: any) => {
    ...
    // Try to use the rate hook to find the best rate
    try {
      // Find the best rate using old rate system parameters
      const bestRate = rateHook.findBestRateFromOldParams(
        lvr.toString(), // Convert to string to ensure compatibility
      ...
      if (bestRate) {
        // Set rates
        setInterestRate(bestRate.rate.toString());
        setComparisonRate(bestRate.comparisonRate.toString());
        
        // Calculate monthly payment
        const monthlyPaymentValue = calculatePmt(
          loanAmount,
          bestRate.rate,
          parseInt(values.loanTerm) * 12
        );
  ...
  // Add the calculateUpfrontCosts function
  const calculateUpfrontCosts = (propertyValue: number, loanAmount: number) => {
    ...
    if (isFirstHomeBuyer) {
      ...
      if (propertyValue <= 600000) {
        ...
        newStampDuty = propertyValue * 0.025;
      } else {
        // Standard rate with slight discount
        newStampDuty = propertyValue * (propertyState === "VIC" ? 0.045 : 0.035);
      }
    } else {
      // Standard stamp duty rates (simplified)
      newStampDuty = propertyValue * (propertyState === "VIC" ? 0.055 : 0.04);
    }
    
    setStampDuty(newStampDuty);
    
    // Calculate other charges
    const newOtherCharges = 2000 + propertyValue * 0.001;
  ...
  // Handle proceeding with a selected loan amount
  const handleProceedWithLoanAmount = (selectedAmount: number) => {
    console.log(`Proceeding with loan amount: ${selectedAmount}`);
    // Here you would implement the logic to proceed with the loan application
    // For example, navigate to the next step or open a confirmation modal
  };

  // Function to scroll to the detailed calculations section
  const scrollToDetails = () => {
    const detailsElement = document.getElementById('borrowing-power-details');
    if (detailsElement) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6">
      ...
      <div className="mb-6">
        {/* Conditional rendering based on borrowing power results */}
        {borrowingPowerResults && showBorrowingPowerResults && !viewAffordability ? (
          // Show Borrowing Power Results when available
          <Card className="overflow-hidden border-none shadow-md h-full">
            <CardHeader className="bg-gradient-to-r from-[#f0f7f7] to-white pb-3 flex justify-between items-center">
              <CardTitle className="text-xl text-[#2a7d7d]">Borrowing Power</CardTitle>
              <Button 
                variant="ghost" 
                size="sm" 
                className="text-[#2a7d7d] hover:bg-[#f0f7f7]"
                onClick={() => setViewAffordability(true)}
              >
                View Affordability
              </Button>
            </CardHeader>
            <CardContent className="p-5">
              <BorrowingPowerResultsDisplay
                ...
                onProceed={handleProceedWithLoanAmount}
                isCompact={true}
                onViewDetails={scrollToDetails}
                onOpenLoanDetailsModal={() => setIsLoanDetailsModalOpen(true)}
                onOpenBorrowingPowerModal={() => setIsBorrowingPowerModalOpen(true)}
          ...
          // Show Affordability Card when no borrowing power results
          <Card className="overflow-hidden border-none shadow-md h-full">
            <CardHeader className="bg-gradient-to-r from-[#f0f7f7] to-white pb-3">
            ...
            <CardContent className="p-5">
              <div className="space-y-6">
                ...
                {/* Calculate Borrowing Power CTA */}
                <div className="mt-6 pt-6 border-t border-gray-200">
                  <div className="flex flex-col items-center text-center mb-4">
                    <h3 className="text-lg font-medium text-gray-900 mb-2">Want to know how much you can borrow?</h3>
      ...
      {borrowingPowerResults && (
        <Card className="overflow-hidden border-none shadow-md mb-6" id="borrowing-power-details">
          <CardHeader className="bg-gradient-to-r from-[#f0f7f7] to-white pb-3">
